...
class _LitePlayerScreenState extends ConsumerState<LitePlayerScreen>
  ...
  void _videoListener() {
    ...
    final shouldShowLoading = value.isBuffering || _isStabilizing;
    if (shouldShowLoading != _isLoading)
      setState(() => _isLoading = shouldShowLoading);

    if (value.isPlaying && !value.isBuffering && _isStabilizing) {
      ...
      body: GestureDetector(
        ...
        child: Stack(
          children: [
            ...
            // Top Overlay (Clock)
            if (_showControls && settings.showClock)
              Positioned(
                top: 20,
                right: 30,
                    ...
                    children: [
                      if (widget.streamType == StreamType.live)
                        TVFocusable(
                            ...
                            onPressed: () => _switchChannel((_currentIndex -
                                    1 +
  ...
  void _cycleAspectRatio() {
    ...
    final next = current == 'contain'
        ? 'cover'
        : (current == 'cover' ? 'fill' : 'contain');
