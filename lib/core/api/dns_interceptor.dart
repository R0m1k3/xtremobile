...
class DnsFallbackInterceptor extends Interceptor {
  ...
  bool _isDnsError(DioException err) {
    if (err.type == DioExceptionType.connectionError || err.type == DioExceptionType.unknown) {
       if (err.error is SocketException) {
          ...
          return msg.contains('Failed host lookup') || msg.contains('No address associated with hostname');
       }
    }
    return false;
  }
  ...
  Future<String?> _resolveWithDoH(String hostname) async {
    ...
    String? ip = await _queryDoH('https://8.8.8.8/resolve?name=$hostname&type=A');
    if (ip != null) return ip;

    // 2. Cloudflare DNS (1.1.1.1)
    ip = await _queryDoH('https://1.1.1.1/dns-query?name=$hostname&type=A', 
  ...
  Future<String?> _queryDoH(String url, {Map<String, String>? headers}) async {
    try {
      ...
      final client = HttpClient()
        ..badCertificateCallback = (cert, host, port) => true; // IGNORE SSL ERRORS just for DoH IP connectivity
